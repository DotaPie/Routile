...
function turnPrice(g, a, b, arrival, escape) {
  if (reverses(g, a, b)) {
    /* Free only at a genuine dead end. Where the road ends because we cut it -
       the fetch box, a keep-out zone, the reachability prune - OSM has it
       carrying on, there is no turning head to use, and doubling back is the
       illegal manoeuvre it looks like on the ground. */
    return escape.other || escape.severed
      ? { seconds: UTURN_PENALTY_S, kind: TURN_REVERSAL }
...
export function expandTurns(g, { restricted = null } = {}) {
  ...
  for (let v = 0; v < g.N; v++) {
    ...
    for (let p = g.inStart[v]; p < g.inStart[v + 1]; p++) {
      ...
      const escape = { any: anyEscape, other: otherEscape, severed: g.severed[v] === 1 };
      for (let q = g.outStart[v]; q < g.outStart[v + 1]; q++) {
        const b = g.outArcs[q];
